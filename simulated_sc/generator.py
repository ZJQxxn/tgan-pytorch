...
class FrameSeedGenerator(nn.Module):
    ...
    def __init__(self, z_slow_dim, z_fast_dim):
        ...
        self.dc3 = nn.ConvTranspose1d(128, 128, 4, 2, 1) #TODO: why use this layer?
...
class VideoGenerator(nn.Module):
    def __init__(self, z_slow_dim, z_fast_dim, out_channels=1, bottom_width=100, conv_ch=512):
        super().__init__()
        self.ch = conv_ch
        ...
        self.dc1 = nn.ConvTranspose2d(conv_ch, conv_ch // 2, 4, 4, 1)
        self.dc2 = nn.ConvTranspose2d(conv_ch // 2, conv_ch // 4, 4, 4, 1)
        self.dc3 = nn.ConvTranspose2d(conv_ch // 4, conv_ch // 8, 4, 4, 1)
        self.dc4 = nn.ConvTranspose2d(conv_ch // 8, conv_ch // 16, 4, 4, 1)
...
class Model(nn.Module):
    def __init__(self, z_slow_dim=256, z_fast_dim=256, out_channels=1, bottom_width=100, conv_ch=512):
        super().__init__()
        self.z_slow_dim = z_slow_dim
        self.z_fast_dim = z_fast_dim
    ...
    def forward(self, z_slow):
        ...
        B, n_z_fast, n_frames = z_fast.size() # B: batch size
